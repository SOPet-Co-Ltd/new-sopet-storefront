...
type CheckoutPlatformPromotionModalProps = {
  isOpen: boolean;
  subtotal: number;
  /** Order-wide shipping fee for shipping-type promo preview/batch. */
  shippingFee?: number;
...
export function CheckoutPlatformPromotionModal({
  ...
  const [selection, setSelection] = useState<PlatformPromotionModalSelection>(() =>
    getInitialPlatformPromotionSelection(appliedPromotion),
  );
  const [validatedPromotions, setValidatedPromotions] = useState<PlatformPromotion[]>([]);
  ...
  const footerDiscountAmount = useMemo(() => {
    ...
    return estimatePromotionDiscount(
      selectedPromotion as StorePromotion,
      subtotal,
      cartLines,
      shippingFee,
    );
  }, [cartLines, selectedPromotion, selection, shippingFee, subtotal]);
  ...
  }, [cartLines, error, isOpen, loading, promotions, shippingFee, subtotal, validatePromotions]);
  ...
  return (
    <Modal
      width={712}
      onClose={onClose}
      data-testid="checkout-platform-promotion-modal"
      header={
        <div className="flex w-full flex-col gap-sop-12px">
          <h2 className="sop-body-lg-medium text-sop-neutral-gray-200">เลือกส่วนลดแพลตฟอร์ม</h2>
          <div className="flex items-stretch gap-sop-16px">
            <div className="min-w-0 flex-1">
              <Input
                ...
                data-testid="platform-promotion-code-input"
                onKeyDown={(event) => {
                  if (event.key === 'Enter') {
            ...
            <Button
              ...
              data-testid="platform-promotion-apply-code-button"
            >
          ...
          {showApplyFooter ? (
            <>
              ...
              <Button
                ...
                data-testid="platform-promotion-confirm-button"
              >
                ใช้ส่วนลดนี้
              </Button>
            </>
